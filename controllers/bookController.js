...
const createBook = async (req, res, next) => {
        const {title, author, publisher, genre, pages, ratings, synopsis, imageUrl} = req.body;
        ...
        res.status(201).json({
            success: true, message: "Book data received",
            data: {title, author, publisher, genre, pages, ratings, synopsis, imageUrl}
        });
    } catch {
        res.status(400).json({
            error: true,
